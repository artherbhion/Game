...
class DinoRun extends FlameGame with TapDetector, HasCollisionDetection {
  ...
  void startGamePlay() async{ final tutorialBox = await Hive.openBox<bool>('tutorialBox');
  AudioManager.instance.startBgm('8BitPlatformerLoop.wav');
  bool hasSeenTutorial = tutorialBox.get('hasSeenTutorial', defaultValue: false)!;

  if (!hasSeenTutorial) {
    overlays.add('ImageOverlay'); // Show tutorial
    pauseEngine(); // Pause the game
    await tutorialBox.put('hasSeenTutorial', true); // Mark tutorial as seen
  }
